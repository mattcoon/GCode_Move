...
if len(sys.argv) > 1:
    for argument in sys.argv:
        ...
        match keyword:
            ...
            case '-h':
                ...
                print('           -aAnalyseOnly -lLaserlowerLimit -sScaleXYZ')
    ...
    userIn = input('X Offset ({:}):'.format(str(deltaX)))
    if userIn != '': deltaX = float(userIn) 
    userIn = input('Y Offset ({:}):'.format(str(deltaY)))
    if userIn != '': deltaY = float(userIn) 
    userIn = input('Z Offset ({:}):'.format(str(deltaZ)))
